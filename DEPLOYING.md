...
```
pack build tdemo-python1 --path . --builder ghcr.io/vmware-tanzu/function-buildpacks-for-knative/functions-builder:0.3.1 --env BP_FUNCTION=func.test
```

Where `tdemo-python1` is the name of your runnable function image.

Then run via Docker:

```
docker run -it --rm -p 8080:8080 tdemo-python1
```
...
```
tanzu apps workload create tdemo-python1 -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/tdemo-python1-source \
  --type web
```
...
```
tanzu apps workload get tdemo-python1
```
...
Reference its [documentation](https://github.com/vmware-tanzu/function-buildpacks-for-knative/tree/test/buildpacks/python) for
